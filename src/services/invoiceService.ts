...
export const invoiceService = {
  ...
  async createInvoice(payload: Omit<Invoice, 'id' | 'createdAt'>): Promise<Invoice> {
    ...
    const advancePaid = Math.min(Math.max(0, Number(payload.advancePaid) || 0), totalAmount);
    const remainingBalance = Math.max(0, totalAmount - advancePaid);
    const paymentStatus: PaymentStatus = advancePaid >= totalAmount && totalAmount > 0 ? 'Paid' : 'Pending';
  ...
  async updateInvoice(id: string, updates: Partial<Invoice>): Promise<Invoice> {
    ...
    const totalAmount = Math.max(0, subtotal - discount + tax);
    const advancePaid = updates.advancePaid !== undefined ? Math.min(Math.max(0, Number(updates.advancePaid) || 0), totalAmount) : Math.min(current.advancePaid, totalAmount);
    const remainingBalance = Math.max(0, totalAmount - advancePaid);
    const paymentStatus: PaymentStatus = advancePaid >= totalAmount && totalAmount > 0 ? 'Paid' : 'Pending';
    const businessId = await getBusinessId();
