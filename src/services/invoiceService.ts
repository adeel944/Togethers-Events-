...
async function replaceItems(invoiceId: string, items: InvoiceItem[]) {
  ...
  const { error } = await supabase.from('invoice_items').insert(items.map((item) => ({ invoice_id: invoiceId, description: item.description, quantity: item.quantity, unit_price: item.unitPrice }))); if (error) throw error;
}
...
export const invoiceService = {
  ...
  async createInvoice(payload: Omit<Invoice, 'id' | 'createdAt'>): Promise<Invoice> {
    const businessId = await getBusinessId(); const subtotal = payload.items.reduce((sum, item) => sum + (item.total || item.quantity * item.unitPrice), 0); const discount = Number(payload.discount || 0); const tax = Number(payload.tax || 0); const advancePaid = Number(payload.advancePaid || 0);
    const { data, error } = await supabase.from('invoices').insert({ business_id: businessId, invoice_number: payload.invoiceNumber, document_title: payload.documentTitle || 'BOOKING CONFIRMATION', booking_id: payload.bookingId || null, client_id: payload.clientId, client_name: payload.clientName, client_phone: payload.clientPhone, client_whatsapp: payload.clientWhatsApp, client_email: payload.clientEmail, billing_address: payload.billingAddress, event_type: payload.eventType, event_date: payload.eventDate, event_time: payload.eventTime || null, venue: payload.venue, issue_date: payload.issueDate, due_date: payload.dueDate || null, subtotal, discount, tax, advance_paid: advancePaid, notes: payload.notes || '', terms_and_conditions: payload.termsAndConditions || '', template_id: payload.templateId || 'modern' }).select('*').single();
    if (error) throw error; await replaceItems(data.id, payload.items); const itemMap = await loadItems([data.id]); return mapInvoice(data, itemMap.get(data.id) || payload.items);
  },
  async updateInvoice(id: string, updates: Partial<Invoice>): Promise<Invoice> {
    const current = await this.getInvoiceById(id); if (!current) throw new Error('Invoice not found'); const items = updates.items || current.items; const subtotal = items.reduce((sum, item) => sum + (item.total || item.quantity * item.unitPrice), 0); const discount = updates.discount !== undefined ? Number(updates.discount) : current.discount; const tax = updates.tax !== undefined ? Number(updates.tax) : current.tax; const advancePaid = updates.advancePaid !== undefined ? Number(updates.advancePaid) : current.advancePaid; const businessId = await getBusinessId();
    const dbUpdates: Record<string, any> = { ...(updates.invoiceNumber !== undefined && { invoice_number: updates.invoiceNumber }), ...(updates.documentTitle !== undefined && { document_title: updates.documentTitle }), ...(updates.bookingId !== undefined && { booking_id: updates.bookingId || null }), ...(updates.clientId !== undefined && { client_id: updates.clientId }), ...(updates.clientName !== undefined && { client_name: updates.clientName }), ...(updates.clientPhone !== undefined && { client_phone: updates.clientPhone }), ...(updates.clientWhatsApp !== undefined && { client_whatsapp: updates.clientWhatsApp }), ...(updates.clientEmail !== undefined && { client_email: updates.clientEmail }), ...(updates.billingAddress !== undefined && { billing_address: updates.billingAddress }), ...(updates.eventType !== undefined && { event_type: updates.eventType }), ...(updates.eventDate !== undefined && { event_date: updates.eventDate }), ...(updates.eventTime !== undefined && { event_time: updates.eventTime || null }), ...(updates.venue !== undefined && { venue: updates.venue }), ...(updates.issueDate !== undefined && { issue_date: updates.issueDate }), ...(updates.dueDate !== undefined && { due_date: updates.dueDate || null }), ...(updates.notes !== undefined && { notes: updates.notes || '' }), ...(updates.termsAndConditions !== undefined && { terms_and_conditions: updates.termsAndConditions || '' }), ...(updates.templateId !== undefined && { template_id: updates.templateId }), subtotal, discount, tax, advance_paid: advancePaid };
    const { data, error } = await supabase.from('invoices').update(dbUpdates).eq('id', id).eq('business_id', businessId).select('*').single(); if (error) throw error; if (updates.items !== undefined) await replaceItems(id, items); const itemMap = await loadItems([id]); return mapInvoice(data, itemMap.get(id) || items);
  },
