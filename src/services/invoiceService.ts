...
async function replaceItems(invoiceId: string, items: InvoiceItem[]) {
  ...
  const { error } = await supabase.from('invoice_items').insert(items.map((item) => ({ invoice_id: invoiceId, description: item.description || '', quantity: Number(item.quantity) || 0, unit_price: Number(item.unitPrice) || 0 }))); if (error) throw error;
}
...
export const invoiceService = {
  ...
  async createInvoice(payload: Omit<Invoice, 'id' | 'createdAt'>): Promise<Invoice> {
    const businessId = await getBusinessId(); const items = payload.items || []; const subtotal = items.reduce((sum, item) => sum + (Number(item.total) || Number(item.quantity) * Number(item.unitPrice) || 0), 0); const discount = Number(payload.discount || 0); const tax = Number(payload.tax || 0); const advancePaid = Number(payload.advancePaid || 0); const today = new Date().toISOString().split('T')[0];
    const { data, error } = await supabase.from('invoices').insert({ business_id: businessId, invoice_number: payload.invoiceNumber, document_title: payload.documentTitle || 'BOOKING CONFIRMATION', booking_id: payload.bookingId || null, client_id: payload.clientId || null, client_name: payload.clientName || '', client_phone: payload.clientPhone || '', client_whatsapp: payload.clientWhatsApp || '', client_email: payload.clientEmail || '', billing_address: payload.billingAddress || '', event_type: payload.eventType || null, event_date: payload.eventDate || null, event_time: payload.eventTime || null, venue: payload.venue || '', issue_date: payload.issueDate || today, due_date: payload.dueDate || null, subtotal, discount, tax, advance_paid: advancePaid, notes: payload.notes || '', terms_and_conditions: payload.termsAndConditions || '', template_id: payload.templateId || 'modern' }).select('*').single();
    if (error) throw error; await replaceItems(data.id, items); const itemMap = await loadItems([data.id]); return mapInvoice(data, itemMap.get(data.id) || items);
  },
  async updateInvoice(id: string, updates: Partial<Invoice>): Promise<Invoice> {
    const current = await this.getInvoiceById(id); if (!current) throw new Error('Invoice not found'); const items = updates.items || current.items; const subtotal = items.reduce((sum, item) => sum + (Number(item.total) || Number(item.quantity) * Number(item.unitPrice) || 0), 0); const discount = updates.discount !== undefined ? Number(updates.discount) : current.discount; const tax = updates.tax !== undefined ? Number(updates.tax) : current.tax; const advancePaid = updates.advancePaid !== undefined ? Number(updates.advancePaid) : current.advancePaid; const businessId = await getBusinessId();
