...
export const invoiceService = {
  ...
  async createInvoice(payload: Omit<Invoice, 'id' | 'createdAt'>): Promise<Invoice> {
    ...
    const paymentStatus = advancePaid >= totalAmount && totalAmount > 0 ? 'Paid' : advancePaid > 0 ? 'Partial' : 'Pending';
    ...
    const dbEventDate = normalizeDate(payload.eventDate) || today;
    const dbIssueDate = normalizeDate(payload.issueDate) || today;
    const dbDueDate = normalizeDate(payload.dueDate);
  ...
  async updateInvoice(id: string, updates: Partial<Invoice>): Promise<Invoice> {
    ...
    const subtotal = items.reduce((sum, item) => sum + (Number(item.quantity) * Number(item.unitPrice)), 0);
    const discount = Math.min(updates.discount !== undefined ? Math.max(0, Number(updates.discount) || 0) : current.discount, subtotal);
    const tax = updates.tax !== undefined ? Math.max(0, Number(updates.tax) || 0) : current.tax;
    const totalAmount = Math.max(subtotal - discount + tax, 0);
    const rawAdvancePaid = updates.advancePaid !== undefined ? Math.max(0, Number(updates.advancePaid) || 0) : current.advancePaid;
    const advancePaid = Math.min(rawAdvancePaid, totalAmount);
    const remainingBalance = Math.max(totalAmount - advancePaid, 0);
    const paymentStatus = advancePaid >= totalAmount && totalAmount > 0 ? 'Paid' : advancePaid > 0 ? 'Partial' : 'Pending';
    const businessId = await getBusinessId();
    const dbUpdates: Record<string, any> = { ...(updates.invoiceNumber !== undefined && { invoice_number: String(updates.invoiceNumber || '').trim() }), ...(updates.documentTitle !== undefined && { document_title: updates.documentTitle }), ...(updates.bookingId !== undefined && { booking_id: updates.bookingId || null }), ...(updates.clientId !== undefined && { client_id: updates.clientId || null }), ...(updates.clientName !== undefined && { client_name: updates.clientName || '' }), ...(updates.clientPhone !== undefined && { client_phone: updates.clientPhone || '' }), ...(updates.clientWhatsApp !== undefined && { client_whatsapp: updates.clientWhatsApp || '' }), ...(updates.clientEmail !== undefined && { client_email: updates.clientEmail || '' }), ...(updates.billingAddress !== undefined && { billing_address: updates.billingAddress || '' }), ...(updates.eventType !== undefined && { event_type: normalizeEventType(updates.eventType) }), ...(updates.eventDate !== undefined && { event_date: normalizeDate(updates.eventDate) || current.issueDate || new Date().toISOString().split('T')[0] }), ...(updates.eventTime !== undefined && { event_time: updates.eventTime || '' }), ...(updates.venue !== undefined && { venue: updates.venue || '' }), ...(updates.issueDate !== undefined && { issue_date: normalizeDate(updates.issueDate) || new Date().toISOString().split('T')[0] }), ...(updates.dueDate !== undefined && { due_date: normalizeDate(updates.dueDate) }), ...(updates.notes !== undefined && { notes: updates.notes || '' }), ...(updates.termsAndConditions !== undefined && { terms_and_conditions: updates.termsAndConditions || '' }), ...(updates.templateId !== undefined && { template_id: normalizeTemplateId(updates.templateId) }), subtotal, total_amount: totalAmount, discount, tax, advance_paid: advancePaid, remaining_balance: remainingBalance, payment_status: paymentStatus };
