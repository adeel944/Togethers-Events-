...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const getInvoiceAmount = (invoice: Invoice) => {
    ...
    const itemAmount = (invoice?.items || []).reduce(
    ...
    <div className="space-y-6 sm:space-y-7 animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3.5 sm:gap-4">
        ...
        <div className="glass-panel p-5">
          ...
          <p className="text-[11px] text-slate-500 font-medium mt-1 truncate">{upcomingEvents[0]?.eventDate ? `Next: ${normalizeDate(upcomingEvents[0].eventDate)}` : 'No upcoming dates'}</p>
        </div>
        <div className="glass-panel p-5">
          ...
          {Array.from({ length: 6 }).map((_, index) => {
            ...
            const maxValue = Math.max(
              ...Array.from({ length: 6 }).map((_, i) => {
                ...
                return invoices
                  .filter((invoice) => (invoice.issueDate || invoice.createdAt || '').slice(0, 7) === key)
                  .reduce((sum, invoice) => sum + getInvoiceAmount(invoice), 0);
              }),
              1,
            );
            return (
              <div key={monthKey} className="flex flex-col items-center justify-end h-full gap-2">
                <div className="w-full max-w-20 rounded-t-lg bg-slate-900/15 hover:bg-slate-900/25 transition-all" style={{ height: `${Math.max(value > 0 ? (value / maxValue) * 100 : 3, 3)}%` }} title={`${monthNames[monthDate.getMonth()]}: ${formatMoney(value)}`} />
                <span className="text-[10px] font-bold text-slate-400">{monthNames[monthDate.getMonth()].slice(0, 3)}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass-panel p-6 sm:p-7">
          <div className="flex items-center justify-between mb-5">
            <div><h3 className="text-base font-bold text-[#0f172a]">Upcoming Event Schedule</h3><p className="text-xs text-slate-500 font-medium mt-0.5">Live bookings with future event dates</p></div>
            <button type="button" onClick={() => onNavigate('bookings')} className="text-xs font-bold text-slate-500 hover:text-[#0f172a] flex items-center gap-1">View All <ArrowRight className="w-3.5 h-3.5" /></button>
          </div>
          {upcomingEvents.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="text-[10px] uppercase tracking-widest text-slate-400 font-bold border-b border-white/60"><tr><th className="pb-3">Client</th><th className="pb-3">Event</th><th className="pb-3">Date</th><th className="pb-3 text-right">Amount</th><th className="pb-3 text-right">Status</th></tr></thead>
                <tbody className="divide-y divide-white/50">
                  {upcomingEvents.map((booking) => (
                    <tr key={booking.id} onClick={() => onSelectBooking(booking)} className="hover:bg-white/40 cursor-pointer">
                      <td className="py-3.5 pr-3 font-bold text-xs">{booking.clientName}</td>
                      <td className="py-3.5 pr-3 text-slate-600 text-xs">{booking.eventType}</td>
                      <td className="py-3.5 pr-3 text-slate-500 text-xs">{normalizeDate(booking.eventDate)}</td>
                      <td className="py-3.5 pr-3 text-right font-bold text-xs">{formatMoney(booking.totalAmount)}</td>
                      <td className="py-3.5 text-right"><StatusBadge status={booking.bookingStatus} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="glass-panel p-6 sm:p-7">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-bold text-[#0f172a]">{monthNames[month]} {year}</h3>
            <div className="flex gap-1">
              <button type="button" onClick={() => setCalendarDate(new Date(year, month - 1, 1))} className="p-1.5 rounded-xl bg-white/80 text-slate-500" aria-label="Previous month"><ChevronLeft className="w-3.5 h-3.5" /></button>
              <button type="button" onClick={() => setCalendarDate(new Date(year, month + 1, 1))} className="p-1.5 rounded-xl bg-white/80 text-slate-500" aria-label="Next month"><ChevronRight className="w-3.5 h-3.5" /></button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-7 gap-1 text-center text-xs">
            {Array.from({ length: firstDayOfMonth }).map((_, i) => <div key={`empty-${i}`} className="py-1.5" />)}
            {Array.from({ length: daysInMonth }).map((_, i) => {
              const day = i + 1;
              const hasEvents = getBookingsForDay(day).length > 0;
              const isToday = day === new Date().getDate() && month === new Date().getMonth() && year === new Date().getFullYear();
              return <button type="button" key={day} onClick={() => { const event = getBookingsForDay(day)[0]; if (event) onSelectBooking(event); }} className={`py-1.5 rounded-xl font-semibold ${hasEvents ? 'bg-[#0f172a] text-white cursor-pointer' : isToday ? 'border border-[#0f172a] text-[#0f172a]' : 'text-slate-700'}`}>{day}</button>;
            })}
          </div>
          <div className="mt-6 pt-4 border-t border-white/60">
            <div className="flex items-center justify-between mb-3"><span className="text-[10px] uppercase font-bold tracking-widest text-slate-400">Recent Invoices</span><button type="button" onClick={() => onNavigate('invoices')} className="text-[11px] font-bold text-slate-500">View all</button></div>
            {invoices.length === 0 ? <p className="text-xs text-slate-400">No invoices yet.</p> : <div className="space-y-2.5">{invoices.slice(0, 3).map((invoice) => <div key={invoice.id} className="flex items-center justify-between p-2 rounded-xl bg-white/60 border border-white/80"><div className="min-w-0 pr-2"><p className="font-bold text-xs truncate">{invoice.clientName}</p><p className="text-slate-400 text-[10px] truncate">{invoice.invoiceNumber}</p></div><span className="font-bold text-emerald-600 text-xs">{formatMoney(getInvoiceAmount(invoice))}</span></div>)}</div>}
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel p-6 sm:p-8">
        <div className="flex items-center justify-between mb-5">
          <div><h3 className="text-base font-bold text-[#0f172a]">Recent Client Bookings</h3><p className="text-xs text-slate-500 font-medium mt-0.5">Latest real reservations from the database</p></div>
          <button type="button" onClick={() => onNavigate('bookings')} className="text-xs font-bold text-slate-500 flex items-center gap-1">All Bookings <ArrowRight className="w-3.5 h-3.5" /></button>
        </div>
