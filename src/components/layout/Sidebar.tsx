...
export const Sidebar: React.FC<SidebarProps> = ({
      ...
      {items.map((item) => {
        ...
        return (
          <button
            ...
            className={`w-full group relative flex items-center justify-center px-2 py-2.5 rounded-2xl text-[13px] font-medium transition-all ${
              isActive
          ...
          >
            {isActive && <span className="absolute left-1 w-1 h-5 rounded-full bg-[#0f172a]" />}
            <Icon className={`w-5 h-5 shrink-0 transition-transform group-hover:scale-105 ${isActive ? 'text-[#0f172a]' : 'text-slate-500'}`} />
          </button>
        );
      })}
    </nav>
    ...
    <div className="flex flex-col h-full justify-between p-3 overflow-y-auto">
        ...
        <div className="flex items-center justify-center mb-5 px-1">
          {safeProfile.logoUrl ? (
            <img
              ...
              className="w-11 h-11 object-contain rounded-2xl border border-white/80 bg-white/90 p-1.5 shadow-xs"
            />
          ) : (
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-tr from-[#0f172a] to-slate-700 text-white flex items-center justify-center font-bold text-sm shadow-md shadow-slate-900/10">
              TE
            </div>
          )}
          <button
            ...
            className="md:hidden ml-auto p-2 text-slate-400 hover:text-slate-700 hover:bg-white/80 rounded-xl transition-all"
      ...
      <div className="pt-3 mt-3 border-t border-white/60 space-y-2">
        <button
          ...
          className={`w-full flex items-center justify-center p-2 rounded-xl transition-all ${currentTab === 'settings-profile' ? 'bg-white/95 text-slate-900' : 'text-slate-500 hover:bg-white/60 hover:text-slate-900'}`}
        >
          <div className="w-8 h-8 rounded-xl overflow-hidden bg-slate-900 text-white flex items-center justify-center text-[10px] font-bold">
            {safeProfile.logoUrl ? (
              <img src={safeProfile.logoUrl} alt="" className="w-full h-full object-contain bg-white" />
            ) : (
              safeProfile.ownerName ? safeProfile.ownerName.split(' ').map((n) => n[0]).slice(0, 2).join('') : 'TE'
            )}
          </div>
        </button>
        <button
  ...
  return (
    <>
      <aside className="hidden md:flex flex-col w-20 shrink-0 h-full bg-white/55 backdrop-blur-2xl border-r border-white/80 shadow-[4px_0_24px_rgba(15,23,42,0.02)] select-none no-print">
        {navContent}
      </aside>

      {showMobileDrawer && (
        <div className="fixed inset-0 z-50 md:hidden no-print flex">
          <div className="fixed inset-0 bg-slate-900/25 backdrop-blur-xs transition-opacity" onClick={onCloseMobile} />
