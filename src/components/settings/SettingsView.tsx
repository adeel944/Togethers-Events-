...
import { Building, Sliders, Upload, Check, CreditCard, FileText, Save, CheckSquare, Square, MessageCircle, Mail } from 'lucide-react';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ profile, settings, initialTab, onSaveProfile, onSaveSettings }) => {
  ...
  return <div className="space-y-6 animate-in fade-in duration-150 max-w-5xl">
    ...
    {activeTab === 'invoice' && <form onSubmit={saveSettings} className="space-y-6 text-sm">
      ...
      <section className="p-6 rounded-2xl bg-white/80 border border-slate-200/80 shadow-xs space-y-5">
        <div><h2 className="text-base font-bold text-slate-900 border-b border-slate-100 pb-3 flex items-center gap-2"><MessageCircle className="w-4 h-4 text-emerald-600" />WhatsApp Invoice Message</h2><p className="text-xs text-slate-500 mt-2">This message is sent before the automatic invoice details. The invoice details are always included automatically.</p></div>
        <textarea rows={7} value={settingsForm.whatsappMessageTemplate || DEFAULT_WHATSAPP_MESSAGE} onChange={e => setSettingsForm({...settingsForm, whatsappMessageTemplate:e.target.value})} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm leading-relaxed" />
        <p className="text-[11px] text-slate-500">Available placeholders: <code>{'{{client_name}}'}</code>, <code>{'{{business_name}}'}</code>, <code>{'{{event_type}}'}</code></p>
      </section>

      <section className="p-6 rounded-2xl bg-white/80 border border-slate-200/80 shadow-xs space-y-5">
        <div><h2 className="text-base font-bold text-slate-900 border-b border-slate-100 pb-3 flex items-center gap-2"><Mail className="w-4 h-4 text-slate-600" />Email Invoice Message</h2><p className="text-xs text-slate-500 mt-2">This message is used as the email body. Invoice details are automatically included below it.</p></div>
        <textarea rows={7} value={settingsForm.emailMessageTemplate || DEFAULT_EMAIL_MESSAGE} onChange={e => setSettingsForm({...settingsForm,emailMessageTemplate:e.target.value})} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm leading-relaxed" />
        <p className="text-[11px] text-slate-500">Available placeholders: <code>{'{{client_name}}'}</code>, <code>{'{{business_name}}'}</code>, <code>{'{{event_type}}'}</code></p>
      </section>
